...
In the following example, we run our analyses on a subset of 12 epigenetic
...
stored in the additional package **OMICsPCAdata()** and summarised below:
\  
...
constructor function in this name is provided to create an object of 
...
methylation (leftmost boxplot of the uppermost and leftmost panel). 
...
An example of such a data frame is given below:
\  
...
3. **groups**: a full set or subset of groups created by
***create_group()***. The group names should be provided as a 
...
PCs are 0, the KDE can't be calculated. Here is an  example,
\  
...
A Similar message is appearing here as observed in `choice` = 2.
...
It appears that 
**NOTE :** The following example will generate the same error as 
...
has the same value on PC1 and PC2.
