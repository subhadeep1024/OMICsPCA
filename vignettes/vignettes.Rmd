...
analyze multiple types of heterogeneous assays, factors, data 
...
numbers of samples(e.g. cell lines) or data modalities (e.g. 
...
since the cell lines and the number of cell lines is different
...
Although overcomes the limitation of class association to some 
extent, this process is only sufficient for the applications where
qualitative integration is sufficient.
...
of the source of variation. A common strategy used in such 
studies is to perform a large number of association tests
between the features and the data modalities (5, 6). Alternatively, 
kernel or graph-based methods are also considered as methods of
integration and the integrated data is then used to build common
similarity networks between samples (7, 8). Although effective 
in accomplishing specific tasks, such approaches lack proper 
interpretability (9) and do not suffice to explain many other
relevant queries applicable to integrated omics data. Such 
queries include identification of variably controlled genes, 
genes sharing similar epigenomic state across various samples
and many more. 
\  
...
designed to identify the source of variation among both the 
samples/variables (the columns of a table, e.g. cell lines, 
patients etc.) and individuals (the rows of a table, e.g. genes,
...
and analyzed and the data points or individuals (e.g. genes, TSSs, 
...
**ROADMAP epigenomics project** portal. The data is stored in the additional 
...
# Example
\  
In this section we demonstrate the functionality of OMICsOCA with an example.
We divided the GENCODE annotated TSSs into 4 groups on the basis of their 
on-off state in 31 cell lines and mapped ChIP-seq, DNA methylation, DNase I
hypersensitivity data with them in order to study the consolidated effect of
...
OMICsPCA has an inbuilt function **prepareDataset()** that merges the factors
...
contain the values corresponding to a factor specific to a particular cell
type.
...
files of the same assay (e.g. H3k9ac).

2. **annofile** : full path of the file containing the annotation file
(e.g exons, TSSs, genes etc) in `.bed` format

3. **annolist** : name of full set or subset of the entries in the annotation
file.
...
Here we show the use of **prpareDataset()** by integrating H2az ChIP-seq 
peaks from 5 cell lines into a data frame named "H2az". The example 
data sets are packaged inside the `OMICsPCAdata` package
\  
...
prepareDataset() will combine these 5 cell lines into a dataframe.
...
correspond to the 5 cell lines shown above.
\  
...
into an user-defined S4 variable and then run various analyses on this
object. The class of this S4 object is predefined as **OMICS** and a 
contsructor function in this name is provided to create an object of 
...
by the function InputOMICs. It can read object directly from the
...
To read from a file we need to pass an additional argument path and
set `typeisfile` to TRUE. By default, `typeisfile` is set to FALSE .
The `path` argument should contain the path to the directory
containing files to be read.
\  
...
is prepared, the user may want to divide the entire annotation set
into smaller groups. Here we show an example to divide 154301 GENCODE
...
In the above example the dataframe CAGE_Cell_wise_v2 is divided into
...
data analyses. Even if there is no need to subset any assay to
smaller groups. In such  cases, do the following: 
\  
...
should be supplied as a data frame of a single column(containing the
group names) and with row names through the argument `groupinfo` and the
`predefinedgroups` argument should be set to "TRUE". Here is an example:
\  
...
through argument `factor`. Each panel shows number of boxplots
corresponding to the  group(s) passed through argument `groups`.
Each boxplot in choice 1  represents the distribution of 
percentage of cell lines corresponding to a group that are overlapped 
...
of percentage of celll ines corresponding to a factor/Assay/modality
with various combination of other Assays/factors/modalities
supplied through `factors`.
\  
...
the display of the number of dimensions or use `select.var` in choice 2 to select
...
"NE" (see integrate_variables() step). Therefore doing this analysis might be worth
...
correlated only with PC1. The small value (hence the small arrows) of the 
contribution of other cell lines with PC1 indicates they might be correlated 
with other PCs also. This is more clearly explained when the plot is studied
with the table generated by this command simultaneously.
\  
...
# reading transcript details from a file (GENCODE v17 annotations)
...
functions from  package ggplot2. Additional arugments of
...
A large part of the total variance in the 2 assays (H2az_Cell_wise
...
of H2az_Cell_wise contribute around 7% to PC1. This means that PC1
...
The above analyses clearly indicates the presence of two broad groups
...
"cluster_parameters()" function takes huge memory and CPU for a higher number
of samples. Therefore, it is recommended to use a subset of original data
while running this function. A subset of rows can be selected randomly through
the `rand` argument of "extract()". The user may also choose whether to use
integrated assays or nonintegrated assays through the logical argument
`integrated` of the function "extract()".
\  
